...
class Solver(nn.Module):
    def __init__(self, configs, device=None, pretrained_embed=None):
        ...
        self.project = nn.Linear(600,256) #Word projection into image space
    ...
    def forward(self, x_real, txt_src2trg, txt_lens):
        ...
        style_txt, _ , word_rep  = self.gen.encode_txt(torch.cat(style_src, dim=1), txt_src2trg, txt_lens)
        
        
        print("word_rep", word_rep.shape)
        print("original image size", x_real.shape)
        STOP
        
        #------------------------------------------------------
        '''Single-scale 2-sided attention'''
        word_ = self.project(word_rep) #(24,600) -> (24,256) 
        unnomr_att = content.view(1024, 256) @ word_.T # -> (1024, 24)
        att = unnomr_att/ torch.sum(unnomr_att, dim=1) #keepdim, (1024, 24)
        
        content_new = att @ word_ #  (1024, 24) @ (24,256) -> (1024,256)
        content_new = content_new.permute(1,0).view(1,256,32,32)
        
        #Log attention map
        ''''we want to see the attention over text instead but lets see if images works first'''
        wandb.log({"Attention1": wandb.Image(att.view(24,32,32)[0:1].data)}, commit=True)
    ...
    def gen_update(self, x_real, c_src, c_trg, txt_src2trg, txt_lens, 
        ...
        # decode (cross domain)
        style_txt, logvar_txt , word_rep = self.gen.encode_txt(torch.cat(style_real, dim=1), 
            txt_src2trg, txt_lens)
        
        #------------------------------------------------------
        '''Single-scale 2-sided attention'''
        word_ = self.project(word_rep) #(24,600) -> (24,256) 
        unnomr_att = content_real.view(1024, 256) @ word_.T # -> (1024, 24)
        att = unnomr_att/ torch.sum(unnomr_att, dim=1) #keepdim, (1024, 24)
        ...
        #Log attention map
        ''''we want to see the attention over text instead but lets see if images works first'''
        wandb.log({"Attention2": wandb.Image(att.view(24,32,32)[0:1].data)}, commit=True)
        
        content_real = content_new
        #------------------------------------------------------
    ...
    def sample(self, x_real, txt_src2trg, txt_lens):
        ...
        for i in range(x_real.size(0)):
            ...
            style_txt, logvar_txt,  , word_rep = self.gen.encode_txt(style_real, 
            ...
            #Log attention map
            ''''we want to see the attention over text instead but lets see if images works first'''
            wandb.log({"Attention3": wandb.Image(att.view(24,32,32)[0:1].data)}, commit=True)
    ...
    def dis_update(self, x_real, c_src, c_trg, txt_src2trg, txt_lens, 
        ...
        #Log attention map
        ''''we want to see the attention over text instead but lets see if images works first'''
        wandb.log({"Attention4": wandb.Image(att.view(24,32,32)[0:1].data)}, commit=True)
